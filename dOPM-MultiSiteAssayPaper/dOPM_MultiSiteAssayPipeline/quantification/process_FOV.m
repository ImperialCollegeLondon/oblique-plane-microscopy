...
try
    ...
    for c=1:length(channels)
     fname = [data_dir filesep row col '_tile_' tile '_fused_tp_' tp '_ch_' channels(c) extension];
        assert(isfile(fname)) 
    end
 catch
...
[~,~,membrane]  = bfopen_v([data_dir filesep row col '_tile_' tile '_fused_tp_' tp '_ch_' '0' extension]);
[~,~,sensor]    = bfopen_v([data_dir filesep row col '_tile_' tile '_fused_tp_' tp '_ch_' '1' extension]);
[~,~,nucleus]   = bfopen_v([data_dir filesep row col '_tile_' tile '_fused_tp_' tp '_ch_' '2' extension]);
...
fov_token = [row col '_tile_' tile '_fused_tp_' tp];
...
disp(['nuclei segmentation - done, processing time [min] ' num2str(toc/60)]);
    ...
    dilated_nuc = imdilate(sgm_nuc,se_r); % same as **
    parfor k=1:n_nuc
        k
        z = sgm_nuc==k;
        % collar
        z = imdilate(z,se_r); % same as **
        z = imdilate(z,se_r) &~ z;
        z = z &~ dilated_nuc;
        %
